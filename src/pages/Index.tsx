...
const SignatureList = () => {
  const { signatures, deleteSignature, setActiveSignature, updateSignature, addSignature } = useSignature();
  ...
  const handleAddSignature = async (name: string, content: string) => {
    await addSignature(name, content);
    setIsCreating(false);
  };
